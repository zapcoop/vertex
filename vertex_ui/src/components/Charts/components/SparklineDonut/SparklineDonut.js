...
const createDonutBackground = (width, height, radius, innerRadius, color) => {
  ...
  return (
    ...
    >
      ...
      <circle
        id={`donut-background-${id}`}
        fill={color}
        cx={width / 2}
        cy={width / 2}
        r={radius}
        mask={`url(#donut-mask-${id})`}
        style={{ opacity: 0.1 }}
      />
    </svg>
  );
...
export default class SparklineDonut extends SparklinePie {
  ...
  componentDidMount() {
    ...
    if (this.props.animated) {
      if (this.isChartSimple && !this.animationPlayed) {
        this.chart.on(
          'draw',
          createSimpleDonutAnimation(this.props.animationDuration),
        );
  ...
  render() {
    const donutBackground = createDonutBackground(
      this.props.radius,
      this.props.radius,
      this.props.radius / 2,
    ...
    return (
      <div className={classes.ctDonutChart} {...otherProps}>
        {donutBackground}
        <div className={classes.ctDonutFill} ref="pieContainer" />
      </div>
    );
  }
}
