...
class CollapasablePanel extends React.Component {
  ...
  render() {
    ...
    const optionsColorClass = isCustomColor(this.props.background)
      ? 'text-white'
      : 'text-muted';
    ...
    return (
      <Panel
        maxHeight={
          this.state.collapsed && !!this.state.targetHeight
            ? 0
            : this.state.targetHeight
        }
        footer={
          this.props.footer && (
            <div className={footerClass}>{this.props.footer}</div>
          )
        }
        fullBody
        ref={panel => {
            ...
            <Col md={3} sm={4} className="text-right">
              ...
              >
                {!this.state.collapsed ? (
                  <i
                    className={`fa fa-fw fa-chevron-up ${optionsColorClass}`}
                  />
                ) : (
                  <i
                    className={`fa fa-fw fa-chevron-down ${optionsColorClass}`}
                  />
                )}
              </a>
              <a href="javascript: void(0)" onClick={() => this.closeHandler()}>
                <i className={`fa fa-fw fa-close ${optionsColorClass}`} />
              </a>
            </Col>
          </Row>
        }
      ...
      >
        <div
          className={
            this.props.fill
              ? classes.collapsablePanelBodyNoMargin
              : classes.collapsablePanelBody
          }
        >
          {this.props.children}
        </div>
