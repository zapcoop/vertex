...
const FavoriteStar = props => {
  ...
  return (
    <OverlayTrigger
      ...
      overlay={<Tooltip id="set-favorite-hint">{tooltipText}</Tooltip>}
    >
      <a
        href="javascript:void(0)"
        {...otherProps}
        onClick={() => onFavorited(!favorited)}
      >
        {favorited ? (
          <i
            className="fa fa-fw fa-star fa-lg text-warning"
            style={{ fontSize: '21px' }}
          />
        ) : (
          <i
            className="fa fa-fw fa-star-o fa-lg text-muted"
            style={{ fontSize: '21px' }}
          />
        )}
      </a>
