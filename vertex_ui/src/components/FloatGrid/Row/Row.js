...
import { Row as BootstrapRow } from 'components';
...
class Row extends React.Component {
  ...
  calcColumns(children, initialColumns) {
    ...
    if (this.props.optimalColumns) {
      const getMinCol = (children, colKey) => {
        const colValues = React.Children.map(
          children,
          child => child.props[colKey] || 12,
        );
      ...
      _.keys(columns).forEach(colKey => {
        ...
        let normalizedCols = React.Children.map(
          children,
          child => (child.props[colKey] || 12) / lowestColVal,
        );
        let normalizedColsOk = _.reduce(
          normalizedCols,
          (memo, col) => memo && col % 1 === 0,
          true,
        );
  ...
  calcLayout(children, optimizedColumns, originalColumns) {
    ...
    _.keys(optimizedColumns).forEach(colName => {
      ...
      children.forEach((child, index) => {
        const prevItem =
          currentLayout.length > 0 ? currentLayout[index - 1] : null;
        const prevWidth = prevItem ? prevItem.x + prevItem.w : 0;
        const prevY = prevItem ? prevItem.y : 0;

        const childWidth =
          child.props[colName] || closestDefinedColumn(child.props, colName);

        const childWidthNormalized =
          childWidth * (colValue / originalColumns[colName]);
        ...
        currentLayout.push({
          ...
          // Correct if target x will be larger then max columns available
          x:
            typeof child.props[`${colName}X`] !== 'undefined'
              ? child.props[`${colName}X`]
              : x + childWidthNormalized > colValue ? 0 : x,
          y:
            typeof child.props[`${colName}Y`] !== 'undefined'
              ? child.props[`${colName}Y`]
              : x + childWidthNormalized > colValue ? prevY + 1 : prevY,
          w: childWidthNormalized || undefined,
          h:
            typeof child.props[`${colName}H`] !== 'undefined'
              ? child.props[`${colName}H`]
              : 1,
          minH: 1,
  ...
  getColumnSizes(columnsSizes = this.props.columnsSizes) {
    return columnsSizes && !_.isEmpty(columnsSizes)
      ? columnsSizes
      : Row.defaultProps.columnsSizes;
  }

  componentWillMount() {
    const originalColumns = this.getColumnSizes();
    const optimizedColumns = this.calcColumns(
      this.props.children,
      originalColumns,
    );
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const currentChildren = simplifyChildrenArray(
      React.Children.toArray(this.props.children),
    );
    const nextChildren = simplifyChildrenArray(
    ...
    if (!_.isEqual(nextProps.columnsSizes, this.state.columns)) {
      ...
      if (
        !_.isEqual(
          nextState.columns,
          this.getColumnSizes(nextProps.columnsSizes),
        )
      ) {
    ...
    if (!this.areChildrenSameColumns(currentChildren, nextChildren)) {
      nextState.layouts = this.calcLayout(
        nextChildren,
  ...
  render() {
    ...
    const modifiedChildren = simplifyChildrenArray(
    ...
    return active ? (
      <ResponsiveGrid
        ...
        onLayoutChange={(currentLayout, allLayouts) =>
          this.layoutChanged(currentLayout, allLayouts)}
      ...
      <BootstrapRow>{modifiedChildren}</BootstrapRow>
    );
  }
}
