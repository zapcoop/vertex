...
const renderStatistics = stats => (
  <div>
    <h6 className={classes.sectionTitle}>Statistics</h6>
    <Grid fluid className={classes.sectionContainer}>
      <Row className="m-t-3">
        {_.map(stats.Components, component => (
          <Col sm={4} key={component._id} className="text-center">
            ...
            />
            <h5 className="m-b-0 text-center">{component.Name}</h5>
            <span className="small">{component.Value}%</span>
          </Col>
        ))}
      </Row>
    </Grid>
...
const renderNetwork = network => (
  <div>
    <h6 className={`${classes.sectionTitle} text-center`}>Network</h6>
    <p className="text-nowrap text-center">
      <span className="text-white">
        <span className="small">{`${network.DownStream} KB/s  `}</span>
        <i className="fa fa-level-down text-primary m-r-2" />
        <i className="fa fa-level-up text-primary" />
        <span className="small">{`  ${network.UpStream} KB/s`}</span>
      </span>
    </p>
  </div>
);
...
const renderSystemTable = system => (
  <div className="m-t-2">
    <h6 className={classes.sectionTitle}>
      <i className="fa fa-database m-r-1" />System
    </h6>
    <ListGroup className={classes.list}>
      {_.map(system, sysEntry => (
        <ListGroupItem
          ...
          key={sysEntry._id}
        >
          <span className="small">{sysEntry.Name}</span>
          <span className="small text-white">{sysEntry.Value}</span>
        </ListGroupItem>
      ))}
    </ListGroup>
  </div>
...
const renderLanTable = lan => (
  <div className="m-t-2">
    <h6 className={classes.sectionTitle}>
      <i className="fa fa-wifi m-r-1" />Lan
    </h6>
    <ListGroup className={classes.list}>
      <ListGroupItem
        className={`${classes.listEntry} ${classes.flexSpaceBetween}`}
      >
        ...
        <i className="fa fa-fw fa-sort text-white" />
      </ListGroupItem>
      <ListGroupItem
        className={`${classes.listEntry} ${classes.flexSpaceBetween}`}
      >
      ...
      >
        ...
        <div>
          ...
          <span className="small text-white">{lan.DownStream.Total} MB</span>
        </div>
      </ListGroupItem>
      <ListGroupItem
        className={`${classes.listEntry} ${classes.flexSpaceBetween}`}
      >
        <i className="fa fa-long-arrow-up text-white" />
        <div>
          <span className="small text-white m-r-1">
            {lan.UpStream.Current} KB/s
          </span>
          <span className="small text-white">{lan.UpStream.Total} MB</span>
        </div>
      </ListGroupItem>
    </ListGroup>
  </div>
    ...
    {_.map(machines, (machine, index) => (
      <ListGroupItem key={machine._id}>
        <div className={classes.flexSpaceBetween}>
          <div>
            <i className="fa fa-hdd-o small m-r-1" />
            <span className="text-uppercase small">{machine.Name}</span>
          </div>
          <span className="text-white small">{machine.Performance}</span>
        </div>
        <SlimProgressBar
          className="m-b-1"
