...
const renderUsers = (userGroups, cbUserClicked) => (
  <div>
    {_.toArray(
      _.mapObject(userGroups, (userGroup, groupKey) => (
        <div key={`user-group-${groupKey}`}>
          ...
          <Nav bsStyle="pills" stacked>
            {_.map(userGroup, user => (
              <NavItem key={user._id} href="#" onClick={() => cbUserClicked()}>
                <Media>
                  ...
                  <Media.Body>
                    ...
                    <p className="small text-muted m-b-0">{user.Title}</p>
                  </Media.Body>
                  {user.UnreadMessages ? (
                    <Media.Right align="middle">
                      <Badge
                        bsStyle={
                          user.UnreadMessages.Recent ? 'danger' : 'default'
                        }
                      >
...
class ThirdRightSidebarTab extends React.Component {
          ...
          (message, index) =>
            index % 2 === 0 ? (
              <Media key={message._id}>
                ...
                <Media.Body>
                  <div
                    className={`${message.Recent
                      ? 'bg-primary'
                      : 'bg-gray-dark'} text-white b-r-a-3 p-x-2 p-y-2`}
                  >
                    {message.Content}
                  </div>
                  <h6 className="m-y-1">
  ...
  render() {
    ...
    return (
      <div>
        <h6 className={`${classes.sectionTitle} m-b-2`}>Users in chat</h6>
